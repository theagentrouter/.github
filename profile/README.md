<p align="center">
  <a href="https://theagentrouter.ai">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/theagentrouter/.github/main/profile/agent-router-horizontal-dark.svg">
      <img alt="Agent Router" src="https://raw.githubusercontent.com/theagentrouter/.github/main/profile/agent-router-horizontal.svg" width="420">
    </picture>
  </a>
</p>

<h2 align="center">Models. Tools. Agents.<br>One point of control.</h2>

<p align="center">
  Application teams get one consistent, OpenAI-compatible API for every model and tool, from hosted providers to self-hosted inference.<br>
  Platform teams keep credentials, quotas, routing, failover, and usage — managed centrally, enforced by Envoy.
</p>

<p align="center"><b>Agent Router controls. Envoy carries.</b></p>

<p align="center"><sub>An Agentic AI Foundation project · Built on Envoy · Formerly Envoy AI Gateway</sub></p>

<p align="center">
  <a href="https://theagentrouter.ai/docs/getting-started/">Get Started</a>
  &nbsp;·&nbsp;
  <a href="https://theagentrouter.ai/docs">Docs</a>
  &nbsp;·&nbsp;
  <a href="https://theagentrouter.ai/blog">Blog</a>
  &nbsp;·&nbsp;
  <a href="https://theagentrouter.ai/talks">Talks</a>
  &nbsp;·&nbsp;
  <a href="https://discord.gg/XvSqZZty7k">Discord</a>
</p>

<p align="center">
  <a href="https://github.com/theagentrouter/agent-router/stargazers"><img alt="GitHub stars" src="https://img.shields.io/github/stars/theagentrouter/agent-router?style=flat&logo=github&logoColor=white&color=FF5500&label=Stars"></a>
  <a href="https://github.com/theagentrouter/agent-router/releases/latest"><img alt="Latest release" src="https://img.shields.io/github/v/release/theagentrouter/agent-router?style=flat&color=1A937F&label=Release"></a>
  <a href="https://github.com/theagentrouter/agent-router/blob/main/LICENSE"><img alt="License" src="https://img.shields.io/github/license/theagentrouter/agent-router?style=flat&color=0B3B33&label=License"></a>
  <a href="https://discord.gg/XvSqZZty7k"><img alt="Discord" src="https://img.shields.io/badge/Discord-Join-5865F2?style=flat&logo=discord&logoColor=white"></a>
  <a href="https://x.com/TheAgentRouter"><img alt="X" src="https://img.shields.io/badge/X-%40TheAgentRouter-000000?style=flat&logo=x&logoColor=white"></a>
  <a href="https://agentrouter.substack.com"><img alt="Substack" src="https://img.shields.io/badge/Substack-Subscribe-FF6719?style=flat&logo=substack&logoColor=white"></a>
  <a href="https://www.linkedin.com/company/agent-router"><img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-Follow-0A66C2?style=flat"></a>
</p>

> [!TIP]
> A ⭐ on [`agent-router`](https://github.com/theagentrouter/agent-router) helps other teams find the project. Thank you!

## From laptop to production

Try it with one command on your laptop. Ship the same configuration on Kubernetes — no rewrite in between.

```shell
OPENAI_API_KEY=sk-... aigw run
```

Then point anything OpenAI-compatible at `http://localhost:1975/v1`. → [CLI guide](https://theagentrouter.ai/docs/cli/aigwrun) · [Kubernetes guide](https://theagentrouter.ai/docs/getting-started/)

## Community

<table>
  <tr>
    <td width="180" valign="top"><b>💬 Discord</b></td>
    <td>Questions, ideas, and the maintainers — <a href="https://discord.gg/XvSqZZty7k"><b>join the server</b></a>.</td>
  </tr>
  <tr>
    <td valign="top"><b>🗓️ Weekly meeting</b></td>
    <td>
      <b>Mondays</b> · 8:00 am Pacific · 11:00 am Eastern · 4:00 pm London · 5:00 pm CET<br>
      <sub>Open to everyone. Skipped on US public holidays.</sub><br>
      <a href="https://docs.google.com/document/d/10e1sfsF-3G3Du5nBHGmLjXw5GVMqqCvFDqp_O65B0_w/edit?tab=t.0"><b>Agenda &amp; notes</b></a> ·
      <a href="https://zoom-lfx.platform.linuxfoundation.org/meeting/91546415944?password=61fd5a5d-41e9-4b0c-86ea-b607c4513e37"><b>Zoom</b></a> ·
      <a href="https://zoom-lfx.platform.linuxfoundation.org/meetings/envoy?view=list"><b>Recordings</b></a>
    </td>
  </tr>
  <tr>
    <td valign="top"><b>🛠️ Contribute</b></td>
    <td>
      <a href="https://github.com/theagentrouter/agent-router/blob/main/CONTRIBUTING.md">Contributing guide</a> ·
      <a href="https://github.com/theagentrouter/agent-router/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22">Good first issues</a> ·
      <a href="https://github.com/theagentrouter/agent-router/issues?q=is%3Aissue+label%3Adiscussion">Discussions</a> ·
      <a href="https://github.com/theagentrouter/agent-router/blob/main/SECURITY.md">Security policy</a> ·
      <a href="https://github.com/theagentrouter/agent-router/blob/main/MAINTAINERS.md">Maintainers</a>
    </td>
  </tr>
  <tr>
    <td valign="top"><b>📣 Follow</b></td>
    <td>
      <a href="https://x.com/TheAgentRouter">X</a> ·
      <a href="https://agentrouter.substack.com">Substack</a> ·
      <a href="https://www.linkedin.com/company/agent-router">LinkedIn</a> ·
      <a href="https://theagentrouter.ai/talks">Talks</a>
    </td>
  </tr>
</table>

Using Agent Router? [Add your logo](https://github.com/theagentrouter/agent-router/edit/main/site/src/data/adopters/adopters.json) to the adopters list. We follow the LF Projects [Code of Conduct](https://github.com/theagentrouter/agent-router/blob/main/CODE_OF_CONDUCT.md).

<p align="center">
  <a href="https://star-history.com/#theagentrouter/agent-router&Date">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=theagentrouter/agent-router&type=Date&theme=dark">
      <img alt="Star history of theagentrouter/agent-router" src="https://api.star-history.com/svg?repos=theagentrouter/agent-router&type=Date" width="600">
    </picture>
  </a>
</p>

---

<p align="center">
  <sub>
    Agent Router is an <a href="https://aaif.io">Agentic AI Foundation</a> project, powered by Envoy.
    Copyright © Agent Router a Series of LF Projects, LLC.
    See the <a href="https://theagentrouter.ai/trademark-policy">trademark policy</a> and <a href="https://lfprojects.org">lfprojects.org</a> for terms of use and other project policies.
  </sub>
</p>
